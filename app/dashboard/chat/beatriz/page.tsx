...
// FOTOS DA BEATRIZ
const PHOTOS_DATA = [
  { id: 1, src: '/beatriz/chat/5.jpg', triggerText: "Stop typing. Look at what I'm doing right now..." },
  { id: 2, src: '/beatriz/chat/3.jpg', triggerText: "You liked that? Then you're going to lose your mind with this one." },
  { id: 3, src: '/beatriz/chat/10.jpg', triggerText: "I'm pushing my limits here. Don't show this to anyone, you pig." }
];
...
export default function BeatrizChatPage() {
  ...
  // --- AQUI ESTÁ A LIGAÇÃO À OPENAI ---
  const processBeatrizResponse = async (textInput: string, isPhotoReaction: boolean) => {
    
    setTimeout(async () => {
      let replyContent = "";
      let sendPhoto = false;

      // 1. Verifica se tens pagamentos pendentes
      if (pendingPayment) {
        replyContent = ["Unlock the photo first.", "Stop talking and pay up.", "Money first, honey."][Math.floor(Math.random() * 3)];
      } 
      // 2. Reage a fotos enviadas
      else if (isPhotoReaction) {
        replyContent = getPhotoReaction();
      } 
      else {
        // 3. Verifica se é hora de enviar foto da Beatriz
        ...
        // 4. Se NÃO for hora da foto, pede resposta à IA
        if (!sendPhoto) {
            try {
                const response = await fetch('/api/chat', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ message: textInput, modelId: 'beatriz' })
                });
                const data = await response.json();
                replyContent = data.reply || "I'm busy right now.";
            } catch (error) {
                replyContent = "I can't hear you.";
            }
        }
      }
