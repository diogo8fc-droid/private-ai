...
export default function BeatrizPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-purple-500 selection:text-white pb-20">
      ...
      {/* CSS para a animação do Zoom funcionar */}
      <style jsx global>{`
        @keyframes slowZoom {
          0% { transform: scale(1); }
      ...
      <main className="pt-24 px-6 max-w-4xl mx-auto">
        ...
        <div className="relative w-full bg-gradient-to-r from-purple-900 to-purple-800 border border-purple-500/30 rounded-2xl p-1 mb-8 shadow-2xl cursor-pointer group hover:border-purple-500 transition-all">
            <div className="bg-black/40 rounded-xl py-6 flex flex-col items-center justify-center">
                ...
                <span className="bg-purple-600 text-white px-8 py-2 rounded-full font-bold shadow-[0_0_15px_rgba(147,51,234,0.5)] group-hover:bg-purple-500 transition">
                    GET ACCESS
                </span>
            </div>
        </div>

        {/* --- GALERIA COM MAIS DESFOQUE --- */}
        <div className="grid grid-cols-3 md:grid-cols-4 gap-2 opacity-60 pointer-events-none select-none">
          {[2, 4, 6, 7, 8, 9, 11].map((i) => (
            <div key={i} className="relative aspect-square rounded-lg overflow-hidden bg-neutral-900 border border-white/5">
              <img 
                src={`/beatriz/gallery/${i}.jpg`} 
                alt={`Gallery ${i}`}
                // AQUI ESTÁ A MUDANÇA: blur-2xl
        ...
        <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center animate-[fadeIn_0.3s_ease-out]">
          ...
          <div className="relative w-full h-full md:w-[400px] md:h-[85vh] md:rounded-3xl overflow-hidden bg-neutral-900 shadow-2xl border border-white/10">
            ...
            <div className="absolute bottom-10 left-0 right-0 px-6 z-30">
               ...
               <Link href="/chat/beatriz">
                 <button 
                   className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-4 rounded-full shadow-lg shadow-purple-900/50 transition transform active:scale-95 uppercase tracking-wider text-sm"
                  >
                   Yes, talk to me 🔥
                 </button>
               </Link>
